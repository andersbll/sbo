...
class DictionaryGUI:
	def __init__(self, dic):
		...
		self.gtkBuilder.add_from_file("sboreader_gui.glade")
...
#		self.webBox = self.gtkBuilder.get_object('webBox')
		self.webView = webkit.WebView()
#		self.scrollWindow.remove(self.scrollWindow.child)
...
#		self.checkReverse = self.gtkBuilder.get_object('checkReverse')
		...
		self.btnDetails = self.gtkBuilder.get_object('btnDetails')
		self.treeview = self.gtkBuilder.get_object('treeview')
		
		
#		http://www.pygtk.org/pygtk2tutorial/sec-TreeSelections.html
#		self.treeviewDetails = self.gtkBuilder.get_object('btnDetails')
...
#		self.webView.open('http://www.google.com')
	...
	def search(self):
		...
		print search_terms
		for d in self.dic:
			entry_list = d.lookup(search_terms[0])
#			for e in entry_list:
#				print e
			self.set_search_results(entry_list)
			break
...
#		htmlview = htmltextview.HtmlTextView()
		entry_list_html = ''.join(map(lambda x: x[2], search_results))
#		print entry_list_html
		self.webView.load_html_string(entry_list_html, '')
		print 'woop'
#		contents = ''
#		directions = []
